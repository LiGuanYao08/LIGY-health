...
public class SportController {
    ...
    @Reference
    private SportService diseaseService;
    ...
    //新增功能
    @RequestMapping("/add")
    public Result add (@RequestBody Sport illKonw){
        try {
    ...
    //编辑检查项
    @RequestMapping("/edit")
    public Result edit(@RequestBody Sport illKonw){
        try {
    ...
    public Result queryById(Integer id){
        try {
            Sport sport = diseaseService.findById(id);
            return new Result(true,MessageConstant.FIND_ILLKONW_SUCCESS,sport);
        }catch (Exception E){
